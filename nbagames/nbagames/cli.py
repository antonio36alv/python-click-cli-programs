...
def cli(d):
    ...
    URL = "https://www.cbssports.com/nba/scoreboard/{}/".format(scheduleDate(dayDiff))
    ...
    for game in gameDivs:
        ...
        gameTime = complexScrape(game, "time")
        teamNames = game.find_all("a", class_="team helper-team-name")
        if bool(re.match(r"\b((2[0-2]|0?[1-9]):([0-5][0-9])([AaPp][Mm]))", gameTime)) == False:
            boxScores = boxScore(game)
            if len(boxScores[0]) > 5:
                gameTime += "\tQ1\tQ2\tQ3\tQ4\tOT"
            else:
                gameTime += "\t\tQ1\tQ2\tQ3\tQ4"
            games.append(Game(gameTime, teamNames[0].text.strip(), teamNames[1].text.strip(), boxScores[0], boxScores[1]))
        else:
            overUnder = complexScrape(game, "overUnder")
            spread = overUnder if overUnder == "-" else complexScrape(game, "spread")
        # add to our list of game objects 
            games.append(Game(gameTime, teamNames[0].text.strip(), teamNames[1].text.strip(), overUnder, spread))
